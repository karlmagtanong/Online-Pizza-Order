...
<body>
		...
		<ul class="navbar-nav">
			...
			<li class="nav-item dropdown active">
				...
				<div class="dropdown-menu">
					<a class="dropdown-item active" href="classic.php?pizzaid=1">Classic Pizza</a>
	...
	if(isset($_SESSION['mobileno'])){
		...
		if(validateOrder($conn,$_SESSION['mobileno']) == 1 || validateOrder($conn,$_SESSION['mobileno']) == ""){
			...
			echo "<script>location.href = 'http://localhost/pizza/specialty.php?pizzaid=8'</script>";
	...
	$info = array();
	$result = mysqli_query($conn, "SELECT * FROM pizza_info where info_class in (1)");
	while($row = mysqli_fetch_assoc($result)){
		$info[] = $row;
	...
	<!-- <ul> -->
		...
		<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
			...
			<br>
	<!-- 	<li class="active">
			<a href="#" onclick="changetab('?pizzaid=',<?php echo $val['info_id']?>)"><?php echo $val['info_name']?></a>
				...
				<tr>
					<td style="vertical-align: top;width: 500px">
						...
						<div class="row" style="display: none">
							...
							<h7>Quantity</h7>
							<br>
							<input type="text" name="pizzaclass" value=1>
						</div>
					</td>
...
function checkout(){
	...
	location.href = "http://localhost/pizza/checkout.php?class="
}
