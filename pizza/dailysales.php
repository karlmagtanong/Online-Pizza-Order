...
<body>
	...
	<div class="container">
		...
		<h2>DAILY SALES</h2>
		<br>
		<span style="font-size: 20px;font-weight: bold">TOTAL SALES : </span><span id="totsales"></span>
		<br><br>
		...
		<table class="table">
			...
			while($row = mysqli_fetch_assoc($result)){
				$infoextrassss += getSumpizzas($conn, $row['order_code']) + getToppingsSum($conn, $row['order_code']);
				?>
		...
		<script>
			...
			var totalsales = '<?php echo $infoextrassss ?>';
			...
			$("#totsales").text(numberWithCommas(totalsales)).css({'font-size':'25px','font-weight':'bold'})
